...
Use an installed specialist skill when available and open current official venue instructions.
The orchestrator supplies the frozen scientific contract; a downstream writing or figure skill
must not change the split, metric, baseline, result, or claim scope to improve the story.

When figure production is within an authorized manuscript handoff or working-draft
scope, or the user requests it directly, route new or redesigned scientific schematics
without a separate figure confirmation. Hand the source-bound objects, arrow meanings,
exact labels, claim limits, and destination policy to
`build-scientific-visualizations`. Select the appropriate conference, journal, compound,
or flowchart mode and follow its `references/image-concept-to-vector.md` default:
destination-style GPT Image 2.5 concept → inspection → native editable PPTX
reconstruction → rendered comparison and representative edit test. Include the editable
PPTX and required manuscript exports even if the user did not specify a format.
An established style follows the project brief; otherwise the visualization skill
selects the relevant conference or journal profile.
...
The visualization skill owns model-identity checks, user overrides, native-edit
exceptions, and output QA. If the skill or verified model access is unavailable, keep
generation `not_run` and finish safe content/style preparation. The handoff grants no
new compute, locked-test, canonical-artifact, or external-publication authority.
