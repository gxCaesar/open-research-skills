...
description: Use when selecting, starting, or resuming an evidence-gated research route for a top computer-science venue or Nature-family journal; source-grounded deep reading of one paper or a bounded multi-paper exemplar-corpus calibration; AI-for-biology data feasibility, lane-specific scoop checks, venue fit, cheap-baseline headroom, frozen protocols, controlled iterations, locked-test discipline, claim locking, negative-result handling, manuscript handoff, or curating and auditing the reproducibility of a public research code release. This workflow audits project and experiment evidence rather than manuscript prose, and it does not authorize compute, manuscript mutation, submission, or any external write.
---
...
For a manuscript handoff, route work to the internal paper-reading component or one of
the other public skills when available:
...
Read each selected skill or component before using it. Missing skills are optional
capability gaps, not permission to invent venue rules or scientific evidence.
...
motivation, method, and mechanism figures default to a destination-style
GPT Image 2.5 concept followed by native editable PPTX reconstruction and required
manuscript exports, even without a separate model or format request. Pass the locked
scientific content and destination policy; measured panels retain their source-data
plotting route. Follow the scoped handoff in
