...
description: Use when establishing current policy or authority, organizing evidence, mapping arguments, planning proposal-specific technical-route or figure content, preparing section briefs, reviewing, or validating Chinese research-funding proposals, including NSFC information science and Guangdong natural science funds. This skill owns proposal-specific content planning but hands graphical construction to build-scientific-visualizations. Check the current funder and institutional authoring policy before producing prose; when direct generation is prohibited or unverified, limit work to source verification, evidence maps, section briefs, structural critique, and audit.
---
...
This skill first locks policy, evidence, and the scientific argument. When current
funder and institutional rules permit the requested AI-assisted figure, hand new or
redesigned technical-route, method, architecture, motivation, and mechanism schematics
to `build-scientific-visualizations`. Use `flowchart` for standalone schematics and
`compound-figure` for mixed schematic/data panels. The default is a style-matched
GPT Image 2.5 concept followed by native editable PPTX reconstruction and the required
proposal exports, even when the user names no model or format. Read
