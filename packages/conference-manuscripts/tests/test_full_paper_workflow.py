...
EVALUATION = PACKAGE / "evals" / "full-paper-workflow.json"
REQUIRED_RISK_TAGS = {
    "evidence_conflict",
...
class FullPaperWorkflowEvaluationTest(unittest.TestCase):
    ...
    def test_pressure_evidence_exposes_the_numeric_and_site_scope_conflicts(self):
        ...
        fixtures = {
            Path(relative).name: (PACKAGE / relative).read_text(encoding="utf-8")
    ...
    def test_manual_boundary_retains_the_matched_icml_route_and_authority_state(self):
        ...
        intake = (PACKAGE / manual["fixture_paths"][0]).read_text(encoding="utf-8")
        for line in (
