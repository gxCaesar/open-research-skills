...
Read [source basis](references/source-basis.md) when a venue or reporting guideline matters. Current
target-venue and study-type instructions override generic guidance. If essential
...
Read [design and units](references/design-and-units.md). For every claim-bearing analysis, record:
...
Start from the [analysis-register example](examples/analysis-register.example.json).
Replace every synthetic field with observed manuscript or protocol facts. With
`SKILL_DIR` set to the installed skill directory, run:
...
Read [failure modes](references/failure-modes.md) for specific risk patterns. If a potential issue
...
Read [reporting and figures](references/reporting-and-figures.md). Each reported aggregate gives its own
