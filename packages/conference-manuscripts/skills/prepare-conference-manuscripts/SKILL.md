...
When manuscript work requires a new or redesigned flowchart, architecture, motivation,
method or other scientific schematic, use `build-scientific-visualizations` and its
default **GPT Image 2.5 concept, then native editable PPTX** workflow. Carry the
conference-appropriate style, locked content and insertion width into that handoff;
no separate image-first or PowerPoint request is needed. Measured plots remain
data-derived. Follow the visualization skill's model-availability and policy handling.
If that skill is not installed, prepare the figure brief and report the missing
dependency without silently replacing its workflow. Read-only audits do not authorize
new drawing or modification of an approved figure.

This skill owns conference prose, current venue compliance, response writing, and the
submission package. `research-publication-pipeline` owns scientific design, experiment
execution, frozen evaluation, and claim lock. `build-scientific-visualizations` owns
figure construction and visual QA. This skill may route work to them but must not
silently absorb their authority.
