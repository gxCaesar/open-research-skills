...
For journal work, retain the exact journal/article type/stage and editor instructions.
Use [drafting and polish](drafting-and-polish.md) for the section's writing job and
[section-specific writing](nature-section-style.md) where relevant. Include Extended
Data only where applicable. During a revision, reconcile response, clean and marked
locations through [revision response](revision-response.md); section completion alone
does not complete `editor-pack` or `final-package`.
