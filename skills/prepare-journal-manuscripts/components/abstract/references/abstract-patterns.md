...
`missing coverage or invalid comparison → sampling and annotation design → scale and independent units → evaluation dimensions → finding → demonstrated use within the sampled scope`
...
5. Does the abstract convey the supported answer and its scope? Place a material
   limitation beside the result it qualifies; the closing sentence can state what
   that result enables within the evaluated setting.
