...
- [Runnable multi-modal page](#runnable-multi-modal-page)
...
## Runnable multi-modal page

The [original 18-panel example](../examples/multimodal-18-panel/README.md) includes
six synthetic source tables, a generator, an independently runnable renderer,
vector PDF/SVG, PNG preview and an exact legend. Its 183 × 235 mm teaching canvas
is not a claim of final-journal fit. It contains no real biological findings.

Use these implemented constructions when relevant:

| Page pressure | Concrete construction in the example | Check at final size |
|---|---|---|
| Spatial overview and small ROI need different magnification | a/b share source coordinates, state shapes and colours; one ROI rectangle links the views | Crop membership, aspect ratio and scale bar agree; connector does not obscure the marks |
| Eighteen panels cannot all have the same shape | Five scientific bands allocate 4/3/4/4/3 panels with deliberately different widths | Letters define independently referenced views; subviews do not invent extra experiments |
| Long molecular names consume the plotting rectangle | i reserves 34 mm inside a 72 mm panel for labels rather than shrinking type | Full feature identity and per-feature n remain legible |
| Molecular modalities have incompatible numerical units | g explicitly standardizes by each assay's control SD; raw-unit views retain separate axes | Standardized patterns are not described as absolute abundance comparisons |
| Controls and incomplete linkage compete with attractive effects | h shows missingness, n/r show different nulls, p/q show unit counts and linkage sensitivity | A null's transformation matches its question; exclusion changes remain visible |

For 24 or more panels, apply the same page/group/view hierarchy rather than extending
this example into a mechanical six-column grid. First quantify each panel's actual
label, mark and legend footprint. Related time points, channels or cell galleries may
form nested views with a shared local key when they support one comparison. Keep
independently cited experiments as separate panels. The evidence and actual final-size
render decide the panel count; a count alone is neither quality nor overload.
