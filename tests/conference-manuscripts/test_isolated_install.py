...
""",
    ...
    def test_local_markdown_links_resolve_from_each_owning_document(self):
        ...
        targets = []
        link = re.compile(r"(?<!!)\[[^]]*\]\(([^)]+)\)")
        documents = [PACKAGE / "docs" / "conference-manuscripts.md", *sorted(SKILL.rglob("*.md"))]
    ...
    def test_readme_skill_dir_commands_name_reachable_local_tools(self):
        ...
        readme = (PACKAGE / "docs" / "conference-manuscripts.md").read_text(encoding="utf-8")
        ...
        with fresh_skill_copy() as skill:
            for relative in targets:
                with self.subTest(relative=relative):
                    ...
                    self.assertTrue((skill / relative).resolve().is_relative_to(skill.resolve()))
