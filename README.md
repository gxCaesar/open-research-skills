...
## 有没有证据说它更好：我们自己测了两次，两次都打平

**[两案证据全文](docs/evidence.md)**

仓库里的 447 个测试和 197 条校验规则证明的是**校验器在工作**，不是**用了它论文会更好**。
后者是一个可以被检验的断言，所以我们检验了它，并且公开结果 —— 尽管结果不支持它。

设计：同一份任务和材料，两个互不知情的执行者，一个额外拿到被测 skill；端点在开跑前冻结，
**两个分支各自的可发表断言都先写下来**；跨模型家族盲评，映射存在评审拿不到的地方。
**对照臂的零假设是「不做 X」，不是「把 X 做错」。**

| | 第一案：打一个可复现的发布包 | 第二案：判断一个方向的数据够不够 |
|---|---|---|
| 主结局 | **平局** —— 两个包都从隔离副本复现出目标数，都零介入 | **平局** —— 两臂都报出「7050 行里联合样本 = 0」，都没掉进「两列都有」的陷阱 |
| 盲评 | 核心平局；4 条确认缺陷全在没用 skill 的那份 | 核心平局；6 条确认缺陷两边都有 |

**结论：在这两个任务上，一个有能力的通用 agent 不用这些 skill 也得到了同样正确的答案。**
skill 可归因的差异是**结构** —— 判词分开记录、击杀点名死在哪一层、报击杀前先跑一次 repair table。
**但第二案里那个结构被填错了**：规定的 repair table 产出了，而它的结论建立在一个未经证实的断言上。
可审计的形状本身也能装错东西，装错之后还更难发现，因为它看起来是合规的。

不能由此说「用了论文会更好」（两案都没测稿件质量），也不能外推到人（两臂都是能力很强的
通用 agent，在无人提示下自己找出了材料里的全部陷阱，这是比人更难的对照）。每案 n=1，材料是合成的。
